...
//
#pragma once
#include <rtc/IAgoraRtcEngine2.h>
...
public:
    ...
    */
     ...
     * @param callTimeout
     *     -1: async mode
     *      >=0: sync mode, timeout in milliseconds
     */
    virtual int setApiCallMode(int syncCallTimeout) = 0;
};
...
public:
    ...
    int startScreenCaptureEx(unsigned int windowId, int captureFreq, const Rect *rect, SCREEN_SHARING_MODE mode = SCREEN_SHARING_NORMAL) {
        if (!rect)
            return setObject("che.video.start_screen_capture", "{\"id\":%u,\"captureFreq\":%d,\"mode\":%d}", windowId, captureFreq, mode);
        else
            return setObject("che.video.start_screen_capture", "{\"id\":%u,\"captureFreq\":%d,\"top\":%d,\"left\":%d,\"bottom\":%d,\"right\":%d}", windowId, captureFreq, rect->top, rect->left, rect->bottom, rect->right, mode);
    }

#elif defined(_WIN32)
    int startScreenCaptureEx(HWND windowId, int captureFreq, const Rect *rect, SCREEN_SHARING_MODE mode = SCREEN_SHARING_NORMAL) {
        if (!rect)
            return setObject("che.video.start_screen_capture", "{\"id\":%u,\"captureFreq\":%d,\"mode\":%d}", (unsigned int)windowId, captureFreq);
        else
            return setObject("che.video.start_screen_capture", "{\"id\":%u,\"captureFreq\":%d,\"top\":%d,\"left\":%d,\"bottom\":%d,\"right\":%d,\"mode\":%d}", (unsigned int)windowId, captureFreq, rect->top, rect->left, rect->bottom, rect->right, mode);
    }
